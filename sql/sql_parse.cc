...
void log_slow_statement(THD *thd)
{
  ...
  */
  ...
  /*
    ...
    skip both long_query_count increment and logging
    if the current statement matches the filter.
    QQ: Perhaps Slow_log increment should still happen here.
    The manual says nothing about filters suppressing Slow_log increment.
  */
      ...
      thd->get_examined_row_count() >= thd->variables.min_examined_row_limit)
  {
    ...
    */ 
    if (!log_slow_enabled_statement(thd) ||
        (thd->variables.log_slow_rate_limit > 1 &&
